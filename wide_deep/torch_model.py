...
class WideDeep(nn.Module):
    ...
    def __init__(self,
        ...
        self.linear_1 = nn.Linear(
            input_emb_dim+len(continuous_cols), self.hidden_layers[0])
        if self.dropout:
        ...
        for i, h in enumerate(self.hidden_layers[1:], 1):
            ...
            if self.dropout:
                setattr(self, 'linear_'+str(i+1)+'_drop',
                        nn.Dropout(self.dropout[i]))

        # Connect the wide- and dee-side of the model to the output neuron(s)
        self.output = nn.Linear(
            self.hidden_layers[-1]+self.wide_dim, self.n_class)
    ...
    def forward(self, X_w, X_d):
        ...
        if self.continuous_cols:
            cont_idx = [self.deep_column_idx[col]
    ...
    def fit(self, dataset, n_epochs, batch_size):
        ...
        for epoch in range(n_epochs):
            ...
            for i, (X_wide, X_deep, target) in enumerate(train_loader):
                X_w = Variable(X_wide)
                X_d = Variable(X_deep)
                y = (Variable(target).float() if self.method !=
                     'multiclass' else Variable(target))
